...
export class LoggingInterceptor implements NestInterceptor {
  ...
  log(start, req) {
    let line = `${new Date(start)} [${Date.now() - start}ms] (${req.headers['x-forwarded-for']} ${req.connection.remoteAddress} ${req.ip}): `
    line += `${req.method} ${req.path} ${JSON.stringify(this.obfuscateData(req.body))}`
    // Excluded query
    console.log(line)
  }
